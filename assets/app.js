// Adiciona um listener para o evento 'DOMContentLoaded', que é disparado quando o DOM (Document Object Model) é carregado completamente.
document.addEventListener('DOMContentLoaded', () => {
    const productForm = document.getElementById('productForm');
    const downloadExcelButton = document.getElementById('downloadExcel');
    const itemList = document.getElementById('itemList');
    const searchForm = document.getElementById('searchForm'); // Novo: formulário de busca
    const searchInput = document.getElementById('searchInput'); // Novo: input de busca
    
    let workbook = XLSX.utils.book_new();
    let worksheetData = [['Produto', 'Quantidade', 'Valor Unitário', 'Valor Total']];
    let totalValue = 0;
    let items = []; // Lista para armazenar os itens adicionados
    ...
    productForm.addEventListener('submit', (event) => {
        ...
        if (validateInputs(product, quantity, price)) {
            ...
            const newItem = { product, quantity, price, itemTotalValue }; // Cria um objeto para o item
            items.push(newItem); // Adiciona o item à lista

            addRowToExcel(product, quantity, price, itemTotalValue);
            updateTotalValueOnPage();
            addItemToList(newItem, items.length - 1); // Passa o índice do item
            productForm.reset();
        } else {
    ...
    // Novo: Função para buscar itens na lista
    searchForm.addEventListener('submit', (event) => {
        ...
        if (query) {
            ...
            renderList(filteredItems); // Mostra somente os itens filtrados
        } else {
